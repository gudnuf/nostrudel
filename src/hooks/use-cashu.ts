import { CashuMint, CashuWallet, MeltQuotePayload, MeltQuoteResponse, Proof } from "@cashu/cashu-ts";

const useCashu = () => {
  const addProofsToBalance = async (proofs: Proof[]) => {
    const localProofs = JSON.parse(localStorage.getItem("cashu.proofs") || "[]");

    localStorage.setItem("cashu.proofs", JSON.stringify([...localProofs, ...proofs]));
  };
  ...
  const getWallet = () => {
    ...
    if (!keyset.mintUrl) throw new Error("Wallet not initialized. Add a mint first.");
  ...
  const customGetMeltQuote = async (
    ...
    if (typeof data?.amount !== "number" || typeof data?.fee_reserve !== "number" || typeof data?.quote !== "string") {
      throw new Error("bad response");
    }
